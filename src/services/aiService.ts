...
// 系统提示词的权威版本在 Cloudflare Worker 服务端（worker/src/index.ts），
// 服务端会丢弃客户端传来的 system 消息并注入自己的版本（防篡改）。
// 此处保留客户端提示词，仅作为「尚未重新部署 Worker」期间的过渡兜底：
// 旧版 Worker 原样转发，若客户端不发 system，AI 会失去动漫店铺助手的设定。
...
const WORKER_URL =
  import.meta.env.VITE_AI_WORKER_URL || 'https://japan-map-ai.9dk9jptv8h-hue.workers.dev'
